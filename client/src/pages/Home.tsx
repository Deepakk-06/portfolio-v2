    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative min-h-screen flex flex-col justify-center px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full pt-28 pb-20">
          ...
          <motion.div {...appear(0.05)} className="mb-9">
            <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-white/10 bg-white/[0.04] text-[11px] font-medium text-white/50 tracking-wide">
          ...
          <motion.div {...appear(0.46)}>
            <div className="flex flex-wrap gap-8 pt-8 border-t border-white/[0.07]">
              {[
                ...
                <div key={s.label}>
                  <div className="text-2xl font-bold text-white/85 tracking-tight leading-none mb-1">{s.value}</div>
                  <div className="text-[11px] text-white/30 font-medium uppercase tracking-widest">{s.label}</div>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <section id="about" className="py-28 px-6 border-t border-white/[0.06]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mt-14 grid md:grid-cols-5 gap-12 items-start">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1, ease }} className="md:col-span-3 space-y-5 text-[15px] leading-relaxed text-white/45">
              <p>
              ...
              <div className="pt-3 border-t border-white/[0.06] space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    <div className="text-[13px] font-semibold text-white/65">Project Committee Member — Embedded Systems Club</div>
                    <div className="text-[11px] text-white/30 mt-0.5">New Horizon College of Engineering &nbsp;·&nbsp; Aug 2024 – Sep 2025</div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2, ease }} className="md:col-span-2 grid grid-cols-2 gap-3">
              {[
                ...
                <div key={s.label} className="p-5 rounded-2xl bg-white/[0.03] border border-white/[0.07] hover:border-white/[0.12] transition-colors">
                  <div className="text-2xl font-bold text-white/85 tracking-tight leading-none mb-1.5">{s.value}</div>
                  <div className="text-[10px] font-bold uppercase tracking-widest text-white/28">{s.label}</div>
          ...
          <div className="mt-14">
            {skillsLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[1,2,3,4,5,6].map((i) => <div key={i} className="h-36 rounded-2xl bg-white/[0.03] animate-pulse" />)}
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {skills?.map((cat, idx) => (
                  <motion.div key={cat.id} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: idx * 0.06, ease }} className="p-6 rounded-2xl bg-white/[0.03] border border-white/[0.07] hover:border-white/[0.12] transition-colors">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {cat.items.map((skill) => (
                        <span key={skill} className="text-[11.5px] font-medium px-2.5 py-1 rounded-lg bg-white/[0.06] text-white/55 border border-transparent hover:bg-white/[0.1] hover:text-white/80 transition-all cursor-default font-mono">
                          {skill}
                        </span>
                      ))}
                    </div>
          ...
          <div className="mt-14">
            {projectsLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {[1,2,3,4,5,6].map((i) => <div key={i} className="h-56 rounded-2xl bg-white/[0.03] animate-pulse" />)}
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          <div className="mt-14 grid md:grid-cols-5 gap-12">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1, ease }} className="md:col-span-2 space-y-8">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={item.label} className="flex items-center gap-3.5">
                    ...
                    <div>
                      <div className="text-[10px] uppercase tracking-widest text-white/22 font-semibold mb-0.5">{item.label}</div>
                      {item.href ? (
                        <a href={item.href} className="text-sm text-white/50 hover:text-white/80 transition-colors">{item.val}</a>
                      ) : (
              ...
              <div className="flex gap-3">
                ...
                <a href="https://github.com/Deepakk-06" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/[0.08] bg-white/[0.03] text-white/38 hover:text-white/70 hover:border-white/[0.15] transition-all text-sm">
      ...
      <footer className="py-10 border-t border-white/[0.06]">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-white/18">&copy; {new Date().getFullYear()} — Built with React &amp; Tailwind</p>
          <div className="flex items-center gap-4">
            <a href="https://linkedin.com/in/deepk6" target="_blank" rel="noopener noreferrer" className="text-white/18 hover:text-white/45 transition-colors"><Linkedin className="w-4 h-4" /></a>
            <a href="https://github.com/Deepakk-06" target="_blank" rel="noopener noreferrer" className="text-white/18 hover:text-white/45 transition-colors"><Github className="w-4 h-4" /></a>
            <a href="mailto:deeeeps06@gmail.com" className="text-white/18 hover:text-white/45 transition-colors"><Mail className="w-4 h-4" /></a>
          </div>
        </div>
      </footer>
    </div>
